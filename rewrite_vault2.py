...
dialog_start = content.find('    if (showAddDialog) {', scaffold_start)
if dialog_start == -1:
    # Try finding the photo picker end
    dialog_start = content.find('        if (showAddDialog) {', scaffold_start)
    if dialog_start == -1:
...
"""
    # Find the end of the old function by searching for the end of the file or the next function
    old_item_code_end = content.find('}\n\n', old_item_code_start)
    if old_item_code_end == -1:
        old_item_code_end = len(content)
