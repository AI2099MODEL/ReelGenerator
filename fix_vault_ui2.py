...
# Find the Scaffold block in VaultContentScreen
start_idx = content.find('    Scaffold(\n        topBar = {\n            LedgerTopHeader(')
end_idx = content.find('    // Add Document Dialog', start_idx)

if start_idx != -1 and end_idx != -1:
    old_scaffold = content[start_idx:end_idx]

    new_scaffold = """    Box(modifier = modifier.fillMaxSize().background(
        Brush.verticalGradient(
            colors = listOf(Color(0xFF0F0B18), Color(0xFF1B1430))
        )
    )) {
        Scaffold(
            topBar = {
...
"""
    ...
    # Replace VaultDocumentItem with VaultDocumentCard
...
"""
        ...
        if old_item_code_end != -1:
            content = content[:old_item_code_start] + new_item_code + content[old_item_code_end+3:]
